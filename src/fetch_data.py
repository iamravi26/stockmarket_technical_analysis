...
def fetch_daily_data(company,today_date):
...
daily_data = fetch_daily_data("TCS",today_date)
daily_data = daily_data.dropna(axis=0)
...
daily_data[["SMA","Close"]].plot()
plot.xlabel("Date", size = 20)
plot.ylabel("Values", size = 20)
plot.title(" TCS", size = 25)
plot.show()
